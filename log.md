...
Joe messaged me that the Collision Detection book site was down so I messaged its author Jeff Thompson and then rehosted a [static mirror copy of Collision Detection](https://leetusman.com/CollisionDetection/). This is a fabulous learning tool for learning to code, and to come back to when I need to see some example code. The code examples are in Processing on the site, but the ideas are useful for other languages including JS, Lua, etc. At some point I'd like to port them all to L5. It's a tremendous resource.
