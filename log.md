...
Joe messaged me that the Collision Detection book site was down so I messaged its author Jeff Thompson and then rehosted a [static mirror copy of Collision Detection](https://leetusman.com/CollisionDetection/) and as a [compiled book](https://leetusman.com/archive/collision-detection) on the Dig Archive. This is a fabulous learning tool for learning to code, and to come back to when I need to see some example code. The code examples are in Processing on the site, but the ideas are useful for other languages including JS, Lua, etc. At some point I'd like to port them all to L5. It's a tremendous resource.

Jeff's original site relied on php very minimally, but php is an extra point of failure and the site wasn't currently loading. I notified Jeff but in the meantime I rehosted the site exported as a static site with vanilla HTML, CSS, js, plus it vendors processing.js (1.4.1, which is over a decade old!) and jQuery. Thanks to Jeff for the Creative Commons licensing CC BY-SA. Despite LLM companies abusing this license to ingest into their corpora, then meted out through chats that they charge money for, in addition to other ethical issues, the sharing of these kinds of resources has meant a lot to me and I've learned so much from open source and creative commons licensed works in immeasurable ways.
...
What I'm trying to do, beyond just putting the hours in to practice, is to develop some good patch systems for future performances and recordings featuring the Alto Venova with modular. I'd love to develop a patch somewhat like [Patrick Shiroishi's Mojave Desert Modular Experiment](https://www.youtube.com/watch?v=kOAKsRvt4E4), and listening back, perhaps I have. Though I'd say my sound comes across as a woodwind playing along with itself, which I think is intriguing, and I'd also like to develop a patch that sounds like an electro-acoustic woodwind with glitches and explosions. Reading Patrick's setup I see he used [different modules](https://www.perfectcircuit.com/signal/desert-modular-experiment-patrick-shiroishi), except for the Morphagene, so perhaps I'll try that out later this week.
...
I created [six themes for PyPadio](https://github.com/lee2sman/pyradio-themes), my favorite radio streaming application, which is TUI-based. In ongoing L5 work I am working on a bug fix for rendering custom shapes in L5 and adding in more parameters to receive parity with p5.js and Processing.
...
Unrelated: I created a pipeline for automatically [converting PDF to text](https://gist.github.com/lee2sman/3c38b7767a59e7437cbc7dc93cbf5e05) files, leveraging Tesseract-OCR, aspell spell checker, and sed. It's a well-commented fish shell script. I used it succesfully on a few PDF papers, though it also failed and produced gibberish on a poorly-scanned PDF I had as well.

## 2026-02-09

I created a new starter [Permacomputing NYC website](https://nyc.permacomputing.net/) yesterday and today we held our first Permacomputing NYC meetup, at Ridgewood Commons. It went well, and just about 20 people attended. We started with an hour discussion of permacomputing. We got into it! Really good discussions, explanations, interrogation: software, chips, permaculture definition, degrowth, hardware, reformists vs 'junkers,' and much more. In keeping with the theme, we used a repurposed very old monitor that I plugged my laptop into. Then [Max](https://infrastructures.us/) gave a presentation on [Lichen-markdown](https://lichen.commoninternet.net/) that was itself presented as a site built with Lichen-markdown. Lichen-markdown is a series of software projects with the final iteration as a web-based collaborative space for editing text that makes a website, an alternative to the heavier infrastructure requirements of a Wordpress installation for example. This prompted lots of discussion, and Max presented with humor and humility. Pretty much everyone who attended had ideas to share. This was followed by my own presentation of L5, showing how it worked, features, its implementation, the software stack, a discussion on 'bedrock' platforms, limitations, bugs, planned features and more. There was interest in various IDEs like Max curious about a minimal CLI L5 coding IDE, another person interested in having an L5 extension for VS Code, and also a discussion of a web-IDE possibility. (a year!) or writing a code editor in L5. It was also clear that a fast/hot-reloading feature, what the p5.js web editor calls *tweak* mode, would be nice. Someone else had interest in trying L5 with Fennel (Lua base lisp). There was also a chat about shaders, limitations on older computers. We ended two and a half hours after we started, with the regular circuit hacking night beginning, and plans to try to keep meeting monthly, pending my own organizing energy and regular meeting space secured. Lastly, there was an interest in an email listserve, [now set up via Framagroupes](https://framagroupes.org/sympa/info/permacomputing-nyc), and I have lots of notes for future presentations/discussion topics.
...
<iframe src="https://www.lexaloffle.com/bbs/widget.php?pid=pomodorotimer" allowfullscreen width="621" height="513" style="border:none; overflow:hidden"></iframe>
...
I ended up finishing a working version of [cellarscript](https://notapipe.itch.io/cellarscript) and a first game written in the language. The game is a 2d roguelike dungeon crawler genre game, where you simply are trying to pilot the player to the stairs to descend each level of the dungeon without coming into contact with a creepy-crawly. But, *record skips*, rather than arrow keys or WASD you program a line of code to control the player. And in fact all of the enemies, their movements, colors, and even placement of the stairs are all also programmed in *cellarscript*, the language I created for the jam, a kind of Forth-like stack language. cellarscript is inspired by [PuzzleScript](https://www.puzzlescript.net/) and [Teletype](https://monome.org/docs/teletype/#learning-teletype) most directly, as well as LOGO and Forth and some other language whose name I forget. It was created in less than a week for the jam, so it is not the most robust, but it works and there's a first game written in the language, called [Cellars All the Way Down](https://notapipe.itch.io/cellarscript). I am excited to have a first version, and if I'm feeling up for it or there seems to be more interest, I may go back and improve the artwork, make more levels, or expand the language or make the engine more robust. While the levels and in-game interpreter run cellarscript, the game engine I created was written in L5.

![a grid-based dungeon crawler game]({{"/images/log/L5-cellarscript.webp" | absolute_url}} "A 12 by 12 grid with various colored little entities scattered around")  
